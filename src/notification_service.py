...
class NotificationService:
    ...
    def __init__(self, app, mail, users_collection, config):
        self.app = app
        self.mail = mail
    ...
    def save_frame(self, frame):
        """Save detected fire frame"""
        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        image_path = self.config.DETECTED_FIRES_DIR / f"fire_{timestamp}.jpg"

        cv2.imwrite(str(image_path), frame)

        return image_path

    def send_fire_email(self, frame):
        """Send email alert with fire image"""

        image_path = self.save_frame(frame)

        users = self.users_collection.find({}, {"email": 1})
...
"""
            ...
            try:
                self.mail.send(msg)
                logger.info(f"Fire alert sent to {email}")
            except Exception as e:
